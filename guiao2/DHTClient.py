...
class DHTClient:
    ...
    def put(self, key, value):
        ...
        self.socket.sendto(pickled_msg, self.dht_addr)      # Enviar a chave e o valor para o endereço da dht
...
if __name__ == "__main__":
    client = DHTClient(("localhost", 5000))     # O socket vai ter como endereço 'localhost' e a comunicação vai ser feita na porta 5000
    # add object to DHT (this key is in first node -> local search -> procurar neste nó)
    client.put("A", [0, 1, 2])
    # retrieve from DHT (this key is in first node -> local search)
    print(client.get("A"))

    # add object to DHT (this key is not on the first node -> remote search -> procurar noutros nós)
    client.put("2", ("xpto"))
